(() => {
    ...
    "node_modules/handlebars/dist/cjs/handlebars/compiler/code-gen.js"(exports, module) {
      ...
      CodeGen.prototype = {
        ...
        generateList: function generateList(entries2) {
          var ret = this.empty();
          for (var i = 0, len = entries2.length; i < len; i++) {
            if (i) {
              ret.add(",");
            }
            ret.add(castChunk(entries2[i], this));
          }
          return ret;
        },
  ...
  // js/templates.js
  var annuaireTemplate = `
<form id="filterForm">
    <input type="text" id="nameFilter" value="" placeholder="Nom">
    <select id="departmentFilter">
        <option value="">Tous les d\xE9partements</option>
...
<ul>
    {{#each data.users}}
        <li>
            <h2>{{nom}} {{prenom}}</h2>
            <p>Service / D\xE9partement: {{#each departements}}{{this}}, {{/each}}</p>
        </li>
    {{/each}}
</ul>
{{else}}
    <h1>Aucun r\xE9sultat</h1>
{{/if}}
`;
  ...
  function displayAnnuaire(users, departements) {
    ...
    const template = import_handlebars.default.compile(annuaireTemplate);
  ...
  async function generateDepartements() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      departements.push(departement2);
    }
    return departements;
  }
  ...
  async function search() {
    ...
    let departements = await getDepartement();
    let users = await getEntries();
    if (nameFilter !== "") {
      users = await filterByName(users, nameFilter);
    }
    if (departmentFilter !== "") {
      users = await filterByDepartment(users, departmentFilter);
    }
